  ...
  return (
    <div className="p-4 space-y-4">
      ...
      <input
        ...
        placeholder="종목 검색..."
        className="w-full glass rounded-xl px-4 py-2.5 text-sm outline-none focus:ring-1 focus:ring-violet-500"
      />
        ...
        {CATEGORIES.map(c => (
          <button
            ...
            className={`px-3 py-1.5 rounded-full text-xs font-medium transition-colors ${
              category === c ? 'btn-grad text-white' : 'glass text-slate-400'
            }`}
          >
            {c}
          </button>
      ...
      {adding && (
        ...
        <div className="fixed inset-0 z-[100] bg-black/70 flex items-end">
          <div className="w-full max-w-lg mx-auto glass rounded-t-3xl animate-slideup flex flex-col" style={{ maxHeight: '80vh' }}>
            {/* 헤더 - 항상 보임 */}
            ...
            <div className="overflow-y-auto px-4 pb-8 space-y-3">
              <input
                ...
                placeholder="종목 이름"
                className="w-full glass rounded-xl px-4 py-3 text-sm outline-none focus:ring-1 focus:ring-violet-500"
              />
              <input
                value={newNote}
                onChange={e => setNewNote(e.target.value)}
                placeholder="가이드 / 메모 (선택 · 예: 15회 × 3세트)"
                className="w-full glass rounded-xl px-4 py-3 text-sm outline-none focus:ring-1 focus:ring-violet-500 text-slate-400"
              />
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  {CATEGORIES.slice(1).map(c => (
                    <button
                      ...
                      className={`px-3 py-1.5 rounded-full text-xs font-medium transition-colors ${
                        newCategory === c ? 'btn-grad text-white' : 'glass text-slate-400'
                      }`}
                    >
                      {c}
                    </button>
